...
//loop through available L04 grids to determine smallest location to accomodate slot quantity
    foreach ($L04GridsArray as $key2 => $value) {
        ...
        $var_locvol = intval($var_gridheight * $var_griddepth * $var_gridwidth);

//Call the true fit for L04`
        if ($var_casetf == 'Y' && ($var_PCCHEIin * $var_PCCLENin * $var_PCCWIDin * $var_caseqty > 0)) {
...
echo 'Percent volume remaining for LEVEL ' . $level . number_format(1 - ($L04Vol / $startingl04vol), 4) . '<br>';
...
do {
    ...
    while ($counter <= $maxrange) { //split into 1000 lines segments to insert into table my_npfmvc
        ...
        $SUGGESTED_NEWLOCVOL = ($L04array[$counter]['SUGGESTED_NEWLOCVOL']) / 1000000;
